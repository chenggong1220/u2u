...
function getFilesQueuedCount()
{
	return swfu.getStats().files_queued;
}
...
{
	console.info($('#'+fileid+'_name').text())
	$('#'+fileid.replace(reg,"")+'_state').text(txt);
...
function fileQueued(file)//队列添加成功
{
	...
	selQueue.push(file);
	allSize+=file.size;
	$('#listBody').append('<tr id="'+file.id+'"><td id="'+file.id+'_name">'+file.name+'</td><td>'+formatBytes(file.size)+'</td><td id="'+file.id.replace(reg,"")+'_state">就绪</td></tr>');
