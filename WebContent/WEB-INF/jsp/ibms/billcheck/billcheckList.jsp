		...
		<div class="ibms_form_default">
			<ul>
				<li><font>合同号：</font>
					...
					<li><font>付款状态：</font>
					<h1>
						<select id="payStatus" name="payStatus" class="easyui-combobox" data-options="editable:false">
...
<table id="querygrid" class="easyui-datagrid" title="账单列表"
	...
	<thead>
		<tr>
			...
			<th data-options="field:'currentTerm',width:'10%'">账单期数</th>
			<th data-options="field:'terms',width:'10%'">总期数</th>
			<th data-options="field:'outdateView',width:'10%'">出账日期</th>
			<th data-options="field:'enddateView',width:'10%'">付款截止日</th>
			<th data-options="field:'payStatus',width:'10%',formatter:function(value,rec){if(value==0){return '未支付';}else{return '已支付';}}">付款状态</th>
			<th data-options="field:'operateViewDate',width:'10%'">付款时间</th>
			<th data-options="field:'useTime',width:'10%'">使用时长(小时)</th>
			<th data-options="field:'repairTime',width:'10%'">维修时长(小时)</th>
			<th data-options="field:'rentAmount',width:'10%'">租金</th>
			<th data-options="field:'faxi',width:'10%',formatter:function(value,rec){return '0.00' ;}">罚息</th>
			<th data-options="field:'allAmount',width:'10%'">总费用(元)</th>
			<th data-options="field:'status',width:'10%',formatter:function(value,rec){if(rec.status==true){return '已出账' ;}else{return '未出账';}}">出账状态</th>
			<th data-options="field:'rentType',width:'10%',formatter:function(value,rec){if(rec.rentType==0){return '分时租赁' ;}else{return '分月租赁';}}">租赁类型</th>
		</tr>
	</thead>
</table>
