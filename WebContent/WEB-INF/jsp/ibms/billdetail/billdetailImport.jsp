...
// 		//任务管理
// 		var taskItem = $("a[tlink='/importTask/importTaskList']", window.parent.document);
// 		taskItem.click();
		
// 		taskItem.parent().addClass("expanded");
// 		taskItem.addClass("active");
		
// 		var infoPointItem = $("a[tlink='/asset/dcimAssetList']", window.parent.document);

// 		infoPointItem.parent().removeClass("expanded");
// 		infoPointItem.removeClass("active");
	}
	function pageInit() {
		var uploadurl = "${pageContext.request.contextPath}/web/billdetail/uploadfile;jsessionid=<%=request.getSession().getId()%>",
			ext = 'Excel表格 (*.csv)',
			size = '2 MB',
			count = 5,
			useget = 1,
